...
function Header({ children }: { children: React.ReactNode }) {
    ...
    return <AnimatedPressable onPress={() => accordionIsOpen()}>{children}</AnimatedPressable>;
}

function Icon({ children, rotation = 'clockwise' }: { children: React.ReactNode; rotation?: 'clockwise' | 'counter-clockwise' }) {
    ...
    return (
        <Animated.View style={stylez} layout={LinearTransition.springify().damping(80).stiffness(200)}>
            {children}
        </Animated.View>
    );
}
...
function Sibling({ children, ...rest }: AnimatedComponentProps) {
    return (
        <Animated.View layout={LinearTransition.springify().damping(80).stiffness(200)} {...rest}>
            {children}
        </Animated.View>
    );
}
