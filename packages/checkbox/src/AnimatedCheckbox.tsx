...
import { StyleSheet, TouchableOpacity, View, Text, Platform, AccessibilityInfo, type ViewStyle, type TextStyle } from 'react-native';
import Animated, {
    useSharedValue,
    useAnimatedStyle,
    withSpring,
...
const AnimatedCheckbox: React.FC<AnimatedCheckboxProps> = ({
    ...
    const triggerHapticFeedback = async () => {
        ...
        try {
            // In a real app, you'd use Haptics.selectionAsync()
    ...
    const animatedCheckboxStyle = useAnimatedStyle(() => {
        const animatedBorderColor = interpolateColor(
            progress.value,
            [0, 1],
            [borderColor || '#CCCCCC', 'transparent']
        );
    ...
    const renderCheckbox = () => (
        <Animated.View
            ...
            <Animated.View style={[styles.iconContainer, animatedIconStyle]}>
                <Text style={[styles.icon, { color: iconColor, fontSize: finalIconSize }]}>
                    {getIconName() === 'check' ? '✓' : getIconName() === 'minus' ? '−' : getIconName()}
                </Text>
            </Animated.View>
        </Animated.View>
    );
